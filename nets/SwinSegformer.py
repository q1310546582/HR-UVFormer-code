...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # build blocks 核心SwinTransformerBlock
...
class SwinSegformer(nn.Module):
    ...
    def __init__(self, backbone, input_shape=2000, patch_size=200, num_classes=3,
                 ...
                 window_size=3, mlp_ratio=2., qkv_bias=True, qk_scale=None,
        ...
        self.origin_resolution = input_shape // patch_size
        self.window_size = window_size
        left_pad = self.origin_resolution % self.window_size
        if left_pad != 0:
            self.need_pad = self.window_size - left_pad
            self.patches_resolution = self.origin_resolution + self.need_pad
        else:
            self.need_pad = 0
            self.patches_resolution = self.origin_resolution
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(self.embed_dim * 2 ** i_layer),
                               input_resolution=(self.patches_resolution // (2 ** i_layer),
                                                 self.patches_resolution // (2 ** i_layer)),
    ...
    def forward_features(self, x):
        x = self.embedding(x)
        x = self.zero_padding(x, h=self.need_pad // 2, w=self.need_pad // 2)
        b, n, c = x.size()
        if self.ape:
    ...
    def forward(self, x):
        ...
        x = self.pred_seg(x)
        B,C,H,W = x.size()
        x = x[:,:,self.need_pad // 2:H-self.need_pad // 2, self.need_pad // 2:W-self.need_pad // 2]
        return [None, x]

    def embedding(self, images):
        with torch.no_grad():
            patches = []
            for row in range(self.origin_resolution):
                for col in range(self.origin_resolution):
                    # (B,3,200,200)
                    patch_image = images[:,:,row * self.patch_size: (row + 1) * self.patch_size, col * self.patch_size: (col + 1) * self.patch_size]
                    patch_image = F.interpolate(patch_image, size=(self.backbone.input_shape, self.backbone.input_shape), mode='bicubic', align_corners=True)
            ...
            x = torch.cat(patches,dim=0).transpose(0,1) # (n_patches,B,embedding_dim) => (B,n_patches,embedding_dim)
        return x
    ...
    def zero_padding(self, x, h=1, w=1):
        ...
        x = x.view(-1, C, self.origin_resolution, self.origin_resolution)  # B C H W
        zero_pad_1 = torch.zeros([B, C, self.origin_resolution, w], dtype=torch.float32)
        zero_pad_2 = torch.zeros([B, C, h, self.origin_resolution + 2*w], dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    model = SwinSegformer(backbone, img_size=2000, patch_size=200, num_classes=3)
